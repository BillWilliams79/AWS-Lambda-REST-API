...
def rest_put(put_method, conn, table, body):
    ...
    try:
        ...
        if affected_put_rows > 0:
            ...
            errorMsg = f"HTTP {put_method} failed no rows affected"
            print(errorMsg)
            return compose_rest_response('500', '', "NO DATA SAVED")

    except pymysql.Error as e:
        errorMsg = f"HTTP {put_method} failed: {e.args[0]} {e.args[1]}"
        print(errorMsg)
    ...
    try:
        ...
        if affected_rows > 0:
            ...
            print(f"HTTP {put_method} FAILED to read last_insert_id.")
            return compose_rest_response('201', '', 'CREATED')

    except pymysql.Error as e:
        print(f"HTTP {put_method} FAILED to read last_insert_id: {e.args[0]} {e.args[1]}")
        return compose_rest_response('201', '', 'CREATED')
    ...
    try:
        ...
        errorMsg = f"HTTP {put_method} helper DESC SQL command failed: {e.args[0]} {e.args[1]}"
        print(errorMsg)
        return compose_rest_response('201', '', 'CREATED')
    ...
    try:
        ...
        if row[0][0]:
            ...
            print(f"HTTP {put_method} helper SELECT after WRITE SQL command failed")
            return compose_rest_response('201', '', 'CREATED')

    except pymysql.Error as e:
        errorMsg = f"HTTP {put_method} helper SELECT after WRITE SQL command failed: {e.args[0]} {e.args[1]}"
        print(errorMsg)
        return compose_rest_response('500', '', "errorMsg")
